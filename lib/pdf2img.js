...
var getOptions = function(opts) {
  ...
  options.timeoutMilliseconds = BASE_OPTIONS.timeoutMilliseconds;
  if (opts.hasOwnProperty('timeoutMilliseconds')) {
    options.timeoutMilliseconds = opts.timeoutMilliseconds;
  }
  return options;
};
  ...
  async.waterfall([
    ...
    function (callback) {
      gm(input)
        ...
        .identify("%p ", function (err, value) {
          if (err) {
            if (err.message === 'gm() resulted in a timeout.') {
              return callback({
                result: 'timeout',
                message: 'gm identify call took longer than ' + options.timeoutMilliseconds + 'ms.'
              }, null);
            }
            return callback({
              result: 'error',
              message: 'gm identify: ' + err.message + '.'
            }, null);
          }
    ...
    .write(output, function(err) {
      if (err) {
        if (err.message === 'gm() resulted in a timeout.') {
          return callback({
            result: 'timeout',
            message: 'gm convert call took longer than ' + options.timeoutMilliseconds + 'ms.'
          }, null);
        }
        return callback({
          result: 'error',
          message: 'Can not write output file. gm convert error: ' + err.message
        }, null);
      }
